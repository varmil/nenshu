...
export interface SummaryView {
  /** 1〜3文・全角15〜130字（C17 で改めた）。生成の規格は `pipeline/summary/prompts/generate.md`。 */
  text: string;
}
 ...
 * 「有価証券報告書の実測値（2026年3月期）」の見出しと重なった**——決算期は1画面に1回
 * （S3・Issue #134・`docs/site-chrome/spec.md` 5.1。Issue #128 で「推定」について
 * 決めたのと同じ扱い）。`e2e/data-period.spec.ts` が2回出たところを捕まえた。いまの企業
 * 詳細の決算期は「年収に関するQ&A」の説明と要約の節の説明の2か所で（C16・#838）、ここに
 * 入れると3回目になる。この1行の役目は**何を原文にしたか**で、いつかは Q&A の説明が持っている。
 */
