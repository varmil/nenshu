...
/**
 * 平均残業時間の定義の注記（Issue #224）。
 *
 * **数値は補正しない。定義を明示する。** 女性活躍データベースの算式は
 * 「各月の対象労働者の（法定時間外労働＋法定休日労働）の総時間数の合計 ÷
 * 対象労働者数」で、**起点が法定労働時間（1日8時間・週40時間）にある**。
 * 一方、会社が自社サイトに出す数値は**所定労働時間**を起点にした所定外の
 * 労働時間であることが多く、所定が8時間より短いぶんが構造的に落ちる——三菱商事は
 * 所定が1日7時間15分で、自社公表の31.0時間／月に対してデータベースの値は
 * 全体10.5時間・総合職14.1時間になる（0.75時間／日 × 月20日 ≒ 15時間／月）。
 *
 * **所定労働時間を推定して補正しない。** 有価証券報告書にも女性活躍
 * データベースにも入っておらず、全社ぶんを一次情報から埋められない以上、
 * 補正値は推定値になる（「根拠を隠した推定値を表示しない」）。
 */
export const OVERTIME_DEFINITION_NOTE =
  "この数値は法定労働時間（1日8時間・週40時間）を超えた分の平均です。" +
  "所定労働時間が8時間より短い会社（1日7時間〜7時間45分の会社が多い）では、" +
  "社員の体感や会社が自社サイトで公表している数値より小さく出ます。" +
  "管理職・事業場外みなし労働の適用者は算入されません。";

/**
 * その指標の下に置く定義の注記。持たない指標は空文字。
 *
 * **`WorklifeMetricView` の中に入れない。** 島の props は HTML の属性に直列化
 * されるので、指標ごとに固定の110字を持たせると**同じ文が props と本文の2か所に
 * 出る**（実測で `/company/8058` が +942 B。うち490 B が props ぶんで、残業を
 * 登録していない592社ではその490 B だけが残る）。会社ごとに変わる値ではないので
 * 描画側で引く。
 *
 * **有給・賃金の差異には付けない**（Issue #224 の対象外。spec 2.2）。
 */
export function metricFootnote(key: WorklifeMetricView["key"]): string {
  return key === "overtime" ? OVERTIME_DEFINITION_NOTE : "";
}
