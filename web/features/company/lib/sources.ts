 ...
 *
 * **時点は書かない。** 決算期は「有価証券報告書の実測値」の見出しが持っている
 * （1画面に1回・`docs/site-chrome/spec.md` 5.1）。女性活躍DBの集計時点と分析の参照日も、
 * それぞれの節にある。
 */
...
export function buildSourceRows(presence: PagePresence): SourceRow[] {
  ...
  /*
   * 材料は `/about`「要約と分析の作り方」の4つ。**一般知識も挙げる**——分析の節の断りは
   * 有報・数値・公開資料の3つしか言っていないが、ADR-0015 で一般知識も材料にしてよいと
   * 決めている。出典を並べる場所で1つ落とすと、落としたものが見えなくなる。
   */
  if (presence.analysis) {
    rows.push({
      ...
      label: "AIの評価",
      covers: "「現状と今後」",
      source: ["有価証券報告書・このページの数値・会社の公開資料・AIの一般知識"],
    });
  }
