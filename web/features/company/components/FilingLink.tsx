 ...
 *
 * **実測値の4項目の表の下辺に付ける帯**（Claude Design の案 D）。表の枠の続きとして描き、
 * どの数字の出典かを形で示す——表の側は下の角を丸めず、帯が下の角を持つ。
 *
 * - **帯全体を1つのリンクにする。** 右の「EDINETで開く」だけをリンクにすると、モバイルで
 *   押せる範囲が文字の大きさぶんしかない。高さは `min-h-11`（44px）
 * - **決算期を書かない。** 企業詳細の決算期は真上の見出しと要約の説明の2か所と決まっている
 *   （`docs/site-chrome/spec.md` 5.1）
 * - **社名も書かない。** 真上に社名が並んでいるうえ、長い社名だとモバイルで1行に収まらない
