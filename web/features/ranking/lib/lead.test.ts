...
describe("rankingLead", () => {
  it("社数は見出しと同じ範囲を数える。業種で絞っていれば業種の社数", () => {
    ...
    expect(lead("age=35&ind=海運業")).toBe(
      "2025年3月期〜2026年5月期の有価証券報告書の平均年間給与を、業種の賃金カーブで35歳時点に補正した海運業の9社。従業員100人以上が対象。"
    );
  });

  // 金額の出どころを示す語。年齢そろえでも、モバイルでも出る1文目に入れる。
  it("どちらの表示基準でも有価証券報告書を名乗る", () => {
    for (const query of ["", "age=35", "ind=銀行業", "age=60&ind=銀行業"]) {
      expect(lead(query), query).toContain("有価証券報告書の平均年間給与");
    }
  });
