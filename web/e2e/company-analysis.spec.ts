...
test.describe("有報の要約と AI 分析", () => {
  test("AC-29: 2つが別の節として、決めた位置に出る", async ({ page }) => {
    ...
    /*
     * **並びを固定する**（アートボード 8a / 8b）。分析は平均年収カード → レーダーの後ろ
     * （C15・#821 でカードとレーダーを入れ替えたので、レーダーの直後）で残業・有給の節の前、
     * 要約は稼ぐ力の推移の後ろ・このページの出典の前。位置を見ないと、ページのどこに
     * 出ていても通ってしまう。
     */
    const headings = await page.locator("h2").allTextContents();
    const at = (text: string) => headings.findIndex((h) => h.startsWith(text));
    expect(at("公開資料による全体像")).toBe(at("株式会社キーエンスの現状と今後") - 1);
    expect(at("株式会社キーエンスの現状と今後")).toBe(at("残業・有給・男女の賃金の差異") - 1);
    expect(at("稼ぐ力の推移")).toBe(at("株式会社キーエンスの有価証券報告書の要約") - 1);
    expect(at("株式会社キーエンスの有価証券報告書の要約")).toBe(at("このページの出典") - 1);
