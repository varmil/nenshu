...
# 分析は推測してよい（ADR-0016）。
#
# **3 = 分析を求職者に向け直した版**（181回目・運営者の指示）。2 の期間に、分析が
# 少しずつ要約へ戻っていた——実測で初期60社と直近60社を比べると**主観の語が
# 109 → 44 に減り、事実の語が 119 → 284 に増え、外部情報の利用が 26% → ほぼ0%**に
# なっていた。**ADR-0015 が明示的に却下した「原文の並べ替え」に滑り落ちていた**ので、
# 規格を組み替えた（`## 分析の規格` の直下に「誰に向けて、何を書くのか」を新設・
# 丸めの細則を `prompts/rounding.md` に分離・外部取得を既定の手順に・本文の1文目は
# 判断から入る・機械ゲートに「逃げの語」の上限）。
#
# **4 = 分析を "So What" に振り切った版**（187回目・運営者の指摘）。3 の期間に書いた
# 42社を監査したところ、**主題1本で一貫していたのは9社だけ**で、25社が灰色・8社が
# 幅出しだった。**最も多い型は「一人当たり経常利益の水準」の1文を主題と関係なく
# 挿し込むもの**（21社）で、次いで採用・募集職種（14社）、残業や有給や在籍年数
# （11社）——**事業や利益で主題を立てたあと、末尾で「求職者向けの実務情報」に
# 切り替わる**という共通の壊れ方だった。**規格に足したのは2つ**——「主題は1つ。
# 刺さらない観点は真であっても捨てる」と、「主張が主・事実は従（"So What" の階段を
# 2段降りる。主節が事実で終わる文は本文に1つまで）」。
#
# **版を上げると `plan --regenerate` が古い版の社を書き直しの対象にする。**
#
# 版5（211回目）。運営者が「有報の事実の羅列が分析の主になっている」と2文を名指し
# した。**どちらも文末は主張の形（「〜抱えている」「〜になっている」）で、版4の
# 「主節が事実で終わる文は1つまで」を通っていた**——述語だけ整えても中身が材料の
# 書き写しなら1段目のままになる。**足した線は字数配分と個数**——事実に割く字数は
# 本文の3分の1まで、1つの文に材料由来の事実を3つ以上並べない、1つの主張を支える
# 事実は1つでよい。確かめ方は「材料に書いてある部分を消して、残りが半分に満たなければ
# それは分析ではない」。
SPEC = "5"
