...
# **角括弧も名前の器。** 有報は計画名を `［SG-2026］` のように全角角括弧で書く
# （`「」` と同じ役割で、こちらのほうが多い会社がある）。
...
def digit_runs(text):
    ...
    **カギ括弧・角括弧の中も数えない。** 固有名詞に数字が入っている型——中期経営計画
    `「変革2030」`、家電の `「S!mplus」`、ロボットの `「R-2000シリーズ」`、有報が
    角括弧で書く `［SG-2026］`——は珍しくなく、**数えると「具体的に書くほど上限に
    食い込む」**という逆向きの圧力になる。上限が止めたいのは**量を並べること**で
    あって、名前を出すことではない。

    **英字とハイフンでつながる型式名も語の一部**（`ＵＳ−２型`・`SG-2026`）。162回目、
    新明和工業の要約は数を1つも書いていないのに `ＵＳ−２型` と `［SG-2026］` だけで
    枠（上限2）を使い切っていた。**見るのはハイフン1文字ぶん手前まで**なので、
    `2025-2027`（数字始まり）や `第2四半期` は依然として数える。
    """
    ...
    for m in _DIGIT_RUN.finditer(folded):
        ...
        if _alpha(before) or _alpha(after):
            continue
        # ハイフン1文字を挟んだ英字も語の一部（`ＵＳ−２型`・`SG-2026`）。
        if before in _HYPHENS and _alpha(folded[m.start() - 2] if m.start() >= 2 else ""):
            continue
        out.append(run)
    return out
