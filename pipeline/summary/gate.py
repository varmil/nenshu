...
3. 残った文で文数（1〜3）と字数（全角15〜130）を見る。外れたら不合格

**文ごとに落とすのは AC-3 がそう決めているため。** 1文だけが原文から離れている型
（キーエンスの「自社工場を持たないファブレス体制」）を、説明文ごと捨てずに済む。
落とした結果1文も残らなければ、そこで初めて不合格になる。

**1文を通すようになったのは C16（#840）から。** C6 の時点では「2〜3文・全角60〜130字」で、
事業の中身が原文に1文しか書かれていない会社（東京海上ホールディングス・くら寿司ほか）は
空にしていた。**「1文にするのは原文の中身が1文ぶんしか無いときだけ」はここでは
判定できない**——原文に中身があるのに1文で済ませた出力は通ってしまうので、
プロンプト（`prompts/generate.md`）と目視で止める。
"""
...
# **下限15字は、業種名の言い換えにしかならない文を落とす線**（C16・#840）。「化学品事業を
# 営む。」（9字）は業種と同じことしか言っておらず、業種はページの別の場所に出ている。
# ベリテの「宝飾品等の小売販売及び卸売販売を行う。」（19字）は通る。
MIN_LEN, MAX_LEN = 15, 130
MIN_SENTENCES, MAX_SENTENCES = 1, 3


def width(text):
    """全角換算の字数。半角は0.5字として数え、切り上げる。

    規格が「全角15〜130字」（spec 1.18）なので、`len()` で数えると欧文の多い会社
    だけが甘くなる。
    """
    n = 0.0
    for c in text:
