...
**落ちた会社を回し直すときは `plan --rejected`**（C16・#840）。CSV で rejected の会社だけを
...
# **書き直しの上限**（C16・#840）。C9 と同じ2回。**通るまで直せるなら、検証パスは
...
def rejected():
    """CSV で rejected になっている会社（C16・#840）。`(原文の行, 前回の CSV の行)` の並び。
...
def cmd_gate(args):
    ...
    **書き直しの回でも、前の回の文と落ちた理由は検証に渡さない**（C16・#840）。
...
def cmd_retry(args):
    """その回で落ちた会社だけを、**落ちた文と理由を添えて**生成側に戻す（C16・#840）。
...
def main():
    ...
    # C16（#840）。CSV で rejected の会社だけを選び、前回の落ちた理由を添える。
