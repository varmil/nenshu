...
注記と同じ性質）。実測142件でタグは0件、実体参照は `&amp;` が69件あった。
**それでもタグを落とす経路を残してあるのは、原本の XBRL（`type=1`）を読む日が来た
ときに変換の規則を2か所に分けないため。**

**段落の切れ目は全角空白（U+3000）として残る。** 改行はCSVの値に1つも無く
（実測142件で0件）、日本語の字下げに使われた U+3000 だけがブロックの境目を示す。
`docs/company/spec.md` AC-18 が「改行は落とさない（段落の切れ目は要約の入力に
効く）」と言っているものは、この形では U+3000 のことになる——**空白を一律に
1個へ潰すと、原文がひと続きの塊になって渡る。**
"""
...
def to_plain_text(value):
    ...
    **全角空白を改行に開く代償**——表を畳んだ跡の穴埋め（`コミュニケーション
    サービスグ　　　　ループ`のような行送りの詰め物）でも行が割れる。実測142件の
    1,906か所を目視で25件抜いたところ3件がこの型だった。**空白1個に潰しても同じ
    ところで語は割れる**ので、割れ方が変わるだけで失うものは無い。段落の切れ目が
    残るぶん、改行に開くほうを採る。
    """
    if not value:
        return ""
