...
**企業ロゴは `logo` 施策**（`docs/logo/`・親 Issue #23・ADR-0008）。**L0（調達、#109）は実装済み、L1（器と表示、#110）は未着手**なので、画面はまだ頭文字マークのまま。
...
- **`/` に `logos.json` を丸ごと渡さない。** L1 は「ロゴの有無」と「縦横比」だけを `companies.json` の行と同じ並びの数値配列に畳んで渡す
