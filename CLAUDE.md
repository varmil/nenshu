...
順序: C0（#51）→ C1（#52）→ U11（#71）→ U8（#53）。

**Issue #21（UI改善・Claude Design の `改善案.dc.html`）に着手中。** アートボード 5a/5b/5c（実測値モード）は U11 で実装済み。残りは後続の Unit に分けてある——**U12**（ランキングのレイアウト刷新: PC2カラム＋左サイドバー絞り込み・行の高密度化 #41・年収バー #24・偏差値列・並び替え・業種チップ・ヘッダへの検索統合）、**C2**（企業詳細のレイアウト刷新: 2カラム・要点・水準が近い会社・分布ヒストグラム・年齢別表・この数字の作り方）、**T0**（平均年収の10年推移 #54。**実装済み**）。計画は `~/.claude/plans/tingly-sleeping-puppy.md`。

**年収バーは「そのページの1位を100%」で正規化する**（デザイン案の「上限2,500万円」は採らない）。ページ・フィルタ・並び替え・表示基準が変わるたびに基準が変わるので、最大値は `rankedCompanies`（ページ切り出し済み）から導き、全体平均の縦線はページ最大を超える場合は描かない。U12 で実装する。

**平均年収の10年推移は `timeseries` 施策**（`docs/timeseries/`）。T0（Issue #74）で `web/public/data/history.json`（1,867社×10年・gzip 65.6KB）まで出してある。表示は C2 に同梱する T1 が担う。

- **EDINET の流量制限は HTTP 200 で来る。** 本文が `{"StatusCode":"429",…}` の JSON になるだけなので `urlopen` は例外を投げない。**中身を検めずに書くと50バイトのエラーJSONが `.zip` としてキャッシュに残り、取得側は成功と数える**（初回は17,719件中5,195件がこれだった）。`edinet.fetch_csv` は先頭が `PK` であることを確かめる。並列は3まで
