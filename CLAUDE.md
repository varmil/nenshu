...
- 取得の失敗はキャッシュしないので、`npm run build:logos` を**走らせ直すたびに数十社ぶん回収される**（429と一時的な失敗）。**ただし回収は先細る**——E3 では1巡目（1,336社）で +875社、2巡目（461社）で +9社だった。到達率 2,507/2,961 = 84.7%、画像は合計13.12MB（1枚平均5.3KB・最大18.4KB）、`logos.json` は gzip 63.6KB
