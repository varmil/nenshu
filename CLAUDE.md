...
- `/company/[id]` の HTML は gzip 8,250 B → 12,873 B（C2）→ 14,223 B（C3）→ **13,993 B**（公開後の2巡目で説明文を落としたぶん）。`/` は変わっていない（`history.json` を `app/page.tsx` から import しないため）
...
- 説明文（`buildCurveSummary`）も要点と同じ線で、**数値から機械的に導ける事実だけ**
