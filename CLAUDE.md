...
**年収偏差値は100を超える。** 35歳時点のキーエンスで150.0（全体平均629万・標準偏差155万に対して2,178万）。年収分布が右に強く裾を引くためで、対数変換しても107.4。**画面には数字だけを出し、水準は順位で読ませる**——「上位◯%」の併記は2026-08-20に運営者の判断で外した（モックに無いものを足さないため）。100を超えうる理由の注記は表の脚注・企業詳細の位置バーの近く・`/about` に残す。**偏差値だけが単独で置かれた画面を作らない**線は変わっていない（glossary参照）。
...
- トップページの HTML は gzip 72,232 B → **71,671 B**。`/company/[id]` は 12,873 B → **13,283 B**（共通ヘッダに検索ボタンが増えたぶん）
