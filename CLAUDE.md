...
- **決算期は `companyFiscalPeriodLabel`。** その会社の決算期で、母集団の幅（`fiscalPeriodLabel`）ではない。spec の AC-22 が `meta.fiscalPeriod` を指したままだったので直した（E1・#172 でその値は `fiscalPeriodRange` に変わっている）
