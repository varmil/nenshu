...
**Cloudflare Workers への自動デプロイは接続済み・稼働中**（https://nenshu.fkmks-247.workers.dev/ ）。**F1（#209）をマージしたら、ダッシュボードのビルドコマンドを `npx opennextjs-cloudflare build` → `npm run build`（＝`astro build`）に変えてもらうこと**——`next` も `@opennextjs/cloudflare` も依存から消えたので旧コマンドは失敗する（ビルドが失敗しても直前の成功バージョンが配信され続けるので本番は落ちない。手順は `docs/ranking/project-foundation/design.md` の2026-08-26追記）。**Astro で `/` 以外はビルド時に生成し、静的アセットとして返す**（ADR-0014・F1）。`/` だけが `export const prerender = false` で、`/about`・`/company/[id]`（全2,961社）・`/sitemap.xml`・`/robots.txt`・`/404.html` はビルド成果物がそのまま並ぶ。デプロイ設定は `docs/ranking/project-foundation/design.md`、移行の構造は `docs/framework/astro-cutover/design.md` 参照。
...
- **ダッシュボードのビルド構成も main に入るまで効かない**（2026-08-26・F1・#209）。ビルドコマンドを変えて保存しても、**ブランチのビルドは旧いコマンドで走る**（4回とも同じ。ビルドの詳細画面の「ビルドの設定」欄で確かめられる）。**「ビルドを再試行」はさらに効かない**——元のビルド構成をそのまま再生する。**ブランチのビルドが使うのはバージョンコマンド（`npx wrangler versions upload`）**で、デプロイコマンドではない
...
- **企業詳細ページの表示基準は URL に出さない。`?age=` は無い**（ADR-0012）。**ランキング（`/`）の `?age=N` は変えていない**——あちらは2,961行の並び順を変え8件をインデックスさせている。配ってしまった `/company/[id]?age=N` は**読まずに `replaceState` で落とす**（落とさないと「URLは30歳・画面は実測値」が残る）
...
**働きやすさ指標は `worklife` 施策**（`docs/worklife/`）。厚生労働省「女性の活躍推進企業データベース」から**平均残業時間・年次有給休暇の取得率・男女の賃金の差異**を取り込み、企業詳細ページに出す。**W0（取り込み・Issue #149）・W1（表示・Issue #150）・W2（異常値の除去・Issue #215）とも実装済み**（`docs/worklife/data-ingest/`・`company-section/`・`outlier-handling/`）。親 Issue #148。**E5（#177）で母集団の拡大に追随させた**（1,548社 → 2,369社 = 80.0%。**W2 で 2,367社 = 79.9%**）。
...
- **「データが無い」が主要な状態**（全2,961社中592社）。`buildWorklifeView(null)` が3指標ぶんの**空の器**を返し、呼び出し側に分岐を作らせない。**節ごとも項目ごとも消さない**——消すと「残業が少ない会社」と見分けがつかない。**「掲載なし」に添える1文は29文字まで**（Issue #192。390pxの本文幅358px・12px の字で1行に収まる上限）——指標名は隣の見出しが持っているので繰り返さない（`有給休暇の取得率` → `取得率`、`男女の賃金の差異` → `差異`）
...
- **トップページの HTML は raw 373,821 B のまま±0**（`src/pages/index.astro` から読まない）。`/company/6861` は 97,889 B → 103,281 B、区分4件＋注釈のある `/company/8058` は 110,254 B。**Issue #191 のバーの組み替えで、区分の多い会社ほど軽くなった**（1本あたり 10 `<span>` → 3 `<span>`。`/company/8058` は 133,477 → 131,266 B）
- **`E2E_BASE_URL` には `http://localhost:<port>` を渡す。`127.0.0.1` にしない。** Next.js の頃は dev origin チェックが一部チャンクを 403 で弾き、**「値は出ているのにクリックが効かない」**（＝ハイドレーションだけ起きない）という形で出た。Astro で同じことが起きるかは確かめていないが、揃えておく理由が無くなったわけではない

- **入力ミスとみられる値は取り込み時に落とす**（W2・#215・親 #185。#207 は重複）。**残業時間と有給取得率の 0**（全体値10社・区分別129セル）——「1か月あたりの平均」が 0 になるのは対象者が1年間まったく残業していないときだけで、**未記入を 0 で埋めたのか本当に 0 なのかを区別できない**。自社の中で矛盾している会社が実在する（花王は全体15.4hなのに区分5件すべて0.0、東洋エンジニアリングは全体0.0なのに総合職8.0h）。残業の軸は小さいほど上位なので、残すとその会社が図の最上位に出る。**代償としてパート・嘱託の本物の 0.0 も消える**。もう1つは**有給の全体値が 100 ちょうどで区分別がすべてそれ未満**のとき（4社。ソニーグループ 100 / 全体 63.7 ほか）——列の見出しが `8.(1)年次有給休暇の取得率-対象労働者(%)` で「対象労働者の割合」とも読めるため、100 は「対象は全労働者」の意味で書かれたとみられる。**100 ちょうど以外は判定しない**（98.8 / 34.5 のような怪しい組み合わせはあるが根拠が無い）。**全体値が区分の範囲外にあることを判定に使わない**——区分は全労働者を覆っているとは限らない（実測で残業50社・有給33社が範囲外だが大半は部分的な登録）。**区分の行そのものは残す**（値だけ空にする）
- **落とした10社では「この会社は残業時間をデータベースに登録していません。」が事実と違う**（登録はしていて、その値を我々が採らなかった）。**直していない**——文言と字数（29文字）は #192 で決めたもので、正しく書き分けるにはデータが「登録が無い」と「採らなかった」を区別する必要がある（`docs/worklife/outlier-handling/design.md`）
...
- **実測（`99_20260825_utf8_bom.zip`＋`Edinetcode.zip`）: 64,963行・236列、法人番号で突合2,641社（89.2%）。** 有報の掲載社すべてに法人番号が入っている（引けない会社は0件）。記入率は賃金の差異84.6%・残業58.1%・有給56.3%（いずれも突合比）
- **母集団を広げると記入率は下がる。** 掲載は任意で、非上場・新規上場の会社は登録自体が無いことが多い（E5 で 61.4% → 58.1%・61.1% → 56.3%）
...
- **`worklife.json` は `/company/[id]` だけが import する。** `src/pages/index.astro` からは読まない（Issue #22）。**表現は `companies.json` と同じ文字列プール＋フラットな配列**——入れ子だと起動コストが倍以上になる（Node 22 実測の中央値で入れ子30.8ms → フラット12.5ms。`companies.json` は6.2ms）。**DuckDB・D1 は使わない**（1,679行の読み取り専用データにDBを持ち込まない）
- **「実測値」とも「推定」とも呼ばない。「自己申告値」**（glossary）——「実測値」は有報の平均年間給与を指す語で衝突する
- **`worklife_2026.csv` は2,367行**（突合2,641社のうち、3指標を1つも持たない274社は行を作らない。W2 の前は2,369行・272社）。`worklife.json` は gzip **186.3KB** で上限220KB。**上限を超えたとき注釈を切り出すかは、Worker バンドルが 3MiB に近づいたかで決める**（`docs/worklife/overview.md`）——E5 の実測でバンドルは gzip 2.089MB（66.4%）なので同梱のまま
- **ZIP は厚労省のオープンデータページから落とす**（`download_b.html?w=99` が全件版）。**curl の既定 User-Agent では 403**（AWS ELB が弾く）。日々更新されるので、**どの版で作ったかは `manifest.json` の sha256 が示す**
...
- **実測: 2,959社（99.9%）。赤字（中央値が0以下）が136社、連結の従業員数が無くて単体で代用したのが371社。** 銀行業82社・保険業17社・その他金融業40社とも欠損なし（営業利益が無いことを理由に欠ける会社は出ない）。**取れない2社は母集団を1.59倍にしても増えていない**（弘電社・キクカワエンタープライズ。最後の開示が8年前）
- **赤字は負のまま持つ。** 捨てると「データが無い」と区別できなくなる
- **業種差が30.7倍ある**（鉱業3,846万円・陸運業125万円）ので**業種中央値を数字として併記する**。**どの業種が端に来るかは母集団で入れ替わる**——E6 の前は 海運業2,524万・輸送用機器131万 の19倍だった。**業種名を決め打ちして倍率を書くと母集団が変わったときに嘘になる**ので、テストは端そのもの（最大 ÷ 最小）を見る。**平均ではなく中央値**——電気機器はキーエンスが業種中央値の18.9倍で、平均だと業種の代表値がキーエンスに引きずられる
...
- **母集団は軸ごとに違う**（平均年収・定着 2,961社 / 稼ぐ力 1,865社 / 有給 1,264社 / 残業 1,413社。有給と残業は W2 で 1,266・1,420 から減った）。**欠測を最下位として数えない**——掲載率6割の軸で、公表している会社が軒並み上位に寄る
...
- **雇用管理区分が2つ以上ある会社は軸に乗せない。** レーダーは1軸に1点しか打てないが、そこで1つ選ぶことは spec 2.2b が禁じた「代表を選ぶ」ことになる。**全体値 → 区分がちょうど1つならその値 → それ以外は乗せない**。キーエンスの有給（区分「正社員」1件）は乗る。**乗せない会社の表記は「掲載なし」ではなく「区分別」**（W2・#215。有給221社・残業114社）——節には値が出ているのに図だけ掲載なしになると、公表していない会社と見分けが付かない。**頂点を打たないことは変えていない**（規則ではなく文言だけを変えた）。**3文字に収める**——リストの値の列は 13px・76px 固定で、超えると右寄せの文字が左へはみ出して隣のラベルを押す。何の区分かは節の説明文が引き受ける
- **平均年収の軸だけが表示基準に追随する**（AC-11）。`radar.json` に入れず、`stats.json` の `rankAll` から `CompanyDetail` が組む——入れると表示基準ごとに9通りの順位を持つことになる
- **モックの「上位◯%」は採らない。** `上位82%` は上から82%の位置の意味だが、日本語としては上位＝良いに読める（キーエンスの有給は実データで1,245/1,264位）。**`1,264社中1,245位` と順位で出す**——偏差値の隣の「上位◯%」を外したのと同じ線
...
- **指標リストの並びは 年収 → 残業 → 有給 → 定着 → 稼ぐ力**（`RADAR_LIST_ORDER`。図の時計回りとは別）。**稼ぐ力だけが2行ぶんの高さを持つ**ので末尾に置く。値は `min-w-[4.75rem]`（76px）・順位は `w-[5.75rem]`（92px）の固定幅で**どちらも右寄せ**、**掲載なしの行も同じ幅の空きを残す**（詰めるとその行だけ右へ寄る）。業種中央値は値の下に右寄せ。**固定幅は最長の文字列より1文字ぶん広く取る**——右寄せの文字は器を超えると**左へはみ出して隣のラベルを押す**ので右端はそろったまま崩れる（順位を84pxにしていたとき実測83pxの `1,867社中1,468位` が1pxしか余らず、フォントの違う環境で崩れた）
...
- **`/company/[id]` の HTML は raw 110,218 B → 125,462 B（+15,244 B）。増分は4社ともほぼ同じ**（棒10本・表10行×4列で、会社によって変わるのは数字の桁だけ）。**`/` は 373,821 B のまま±0**（`profit-history.json` を `src/pages/index.astro` から読まない）。`profit-history.json` は gzip 187.0KB（上限256KB）
...
**平均年収の10年推移は `timeseries` 施策**（`docs/timeseries/`）。T0（Issue #74）で `web/public/data/history.json`（**2,961社×10年・gzip 99.6KB**。E4・#176 で母集団に追随させた）を出し、表示（T1）は C2（#83）に同梱して実装済み。**T2（Issue #138）でグラフの上に数表を足した**（`docs/timeseries/history-table/`）。
...
- **右端が空く会社が349社ある**（E4・#176）。取得の窓を直近12か月に広げた（ADR-0011）ので、決算期が3月でない会社の最新の有報は2025年の提出になる——**2026年を持たないのが正しい。** 途中が欠ける（2117）のとは別の欠け方
- **桁は1行では決まらない。** `run.fix_salary_typos` はあり得る帯に入る10の冪を小さいほうから採るので、**帯が広いぶん間違った桁で止まる**（トスネット2019は提出側の26億円が÷100 の 2,624万円で止まっていた）。10年ぶんを並べればその会社の他の年が正しい桁を指すので、`history.resolve_scale` が選び直す。**浮いて見えるというだけでは直さない**——ホットリンク2018の320万円は有報が「（千円）3,205」と書いている実額
- **帯の上限3,000万円は実在の会社を切る。** Ｍ＆Ａキャピタルパートナーズの2019・2022年（3,109万・3,161万）が÷10 されていた。推移は他の年を基準に戻せるが、**1年ぶんしか持たないランキング側（`run.py`・`unified.py`）は直していない**（いまの実測値は帯の中なので影響は出ていない）
...
- `/company/[id]` の HTML は gzip 8,250 B → 12,873 B（C2）→ 14,223 B（C3）→ 13,993 B（公開後の2巡目）→ **13,824 B**（Issue #95 で表の caption と説明文の3文目を落としたぶん）。**C4（#146）の説明文3か所は raw 97,511 B → 97,889 B（+378 B）**——同一ビルド・同一手順で `next start` に対して前後を測った値（gzip は 14,530 → 14,633 B。起動ごとに揺れるので前後は raw で見る）。`/` は変わっていない（`history.json` を `src/pages/index.astro` から import しないため）
...
**企業ロゴは `logo` 施策**（`docs/logo/`・親 Issue #23・ADR-0008）。**L0（調達、#109）・L1（器と表示、#110）とも実装済み。** **2,961社中2,509社（84.7%）にロゴが出る**（E3・#175 で母集団の拡大に追随させた）。
...
- **その会社では別のURLを使う、と決めたら `candidates.ts` の `PINNED` に書く**（2026-08-26・豊田通商）。`prioritize()` が優先順の先頭に置く。**画像を手で差し替えない**——取り直した瞬間に元へ戻る。豊田通商（8015）は Wikidata の公式サイト（P856）が**英語版**を指しており、そこの `Organization.logo` が 1200×630 の共有用の絵（ロゴは上端の帯だけで残りはグラデーションの余白。`trim` は単色の縁しか落とせない）、英語版ヘッダのロゴは 19.9:1 で器に入らない。**余白の割合で機械的に落とす線は引けなかった**——「インクの外接矩形が画像の半分未満」は2,509枚中52枚あり、その大半は薄い色のワードマークを持つ正しいロゴだった（`docs/logo/logo-pipeline/design.md`）。**同種の見えにくい画像が他にも残っている**（白いワードマークが横に伸びてシンボルだけが見える 3329・6677 など。`blankOnLight` はインク1%以上を通す）
...
- 取得の失敗はキャッシュしないので、`npm run build:logos` を**走らせ直すたびに数十社ぶん回収される**（429と一時的な失敗）。**ただし回収は先細る**——E3 では1巡目（1,336社）で +875社、2巡目（461社）で +9社だった。到達率 2,509/2,961 = 84.7%、画像は合計13.09MB（1枚平均5.3KB・最大18.4KB）、`logos.json` は gzip 62.3KB
- **`meta.count` は母集団の社数（＝CSV の行数）で、今回叩いた社数ではない。** `--only` のとき前回の値を引き継ぐ実装だったため、**E2 で母集団が変わったときに到達率が `2500/1867 = 133.9%` と出た**（E3 で直した）。`missReasons`・`rejectReasons` は**回した社数に対する内訳**で、`partialOf` がその分母を示す
- **新しく入った会社のほうが取りにくい。** 決算期が3月でない会社には非上場・新規上場が多く、**Wikidata に項目が無い**（2巡目の461社のうち271社が「公式サイトURLが無い」）
...
- **マウント時は URL が正。** 戻ったときにサーバーが渡した `initialState` で URL を上書きすると `?page=2` が `/` に戻り、**進む先の履歴まで消える**。Next.js の頃はルーターキャッシュの RSC ツリーがそのまま返って `initialState` が「そのツリーを作ったときのURL」の値になっていた。**Astro でも規則は変えない**——ブラウザのキャッシュから返った HTML も、それが作られたときのURLのものになる
...
- **自分のパスを離れたら書かない・読まない。** Next.js は `pushState` を浅い遷移として扱うので、抜けていくページが行き先のURLを書くと**URLだけ次のページ・画面は前のページ**の行き止まりになっていた（企業詳細 → ランキングの戻るで実際に起きた）。**Astro では文書ごと入れ替わるのでこの経路は無い**が、規則は残してある——コストがほぼ無く、外すと「なぜ外したか」を次に読む人が確かめ直すことになる
...
**掲載企業数の拡大は `expansion` 施策**（`docs/expansion/`・親 Issue #22）。取得の窓を直近12か月に広げて母集団を広げる（ADR-0011）と、全件embed をやめる（ADR-0013）の2つが決まっている。Unit は E0〜E6（Issue #172〜#177・#182）で、**E0〜E6 すべて実装済み**（`docs/expansion/initial-payload/`・`period-range/`・`universe/`。E3・E4・E5・E6 は既存の Unit のデータを作り直しただけなので docs は元の Unit の側にある）。**Unit の ID に `P` は使えない**（`performance` 施策が使っている）。

**母集団は 1,867社 → 2,961社になった**（E2・Issue #173・ADR-0011）。**それまで載っていた1,867社は1社も落ちず、金額も採用書類も1つも変わっていない**（実測）。

- **窓は回す日から遡って12か月**（`run.twelve_month_window`）。**日付を直書きしない。** 以前の 6/1〜7/10 は3月期決算の提出ピークに貼り付いた窓で、キヤノン・JT・楽天・イオン・ファーストリテイリングなど1,095社が落ちていた。作ったデータの窓は `pipeline/data/universe.json` に残る（今回は 2025-08-25〜2026-08-25）
- **1社1件に寄せるキーは EDINETコードで、期末が新しいほう（同じなら `docID` が大きいほう）を採る**（`edinet.doc_rank`）。**「後に見つかったものが勝つ」だと走査順で決まり、同じ年に有報を複数出す会社が消える**——りそな銀行・三井住友信託銀行は窓の中に4件ずつ持ち、従業員の状況を持たない書類が残って実際に消えていた
- **証券コードの有無で経路を分けない。** `run.unlisted_docs()` は消した。**提出者種別（内国法人・組合）の絞り込みは上場側にも当たる**——以前は非上場側の経路だけが持っていた
- **`pipeline/data/universe.json` は「CSV の行からは出せないもの」だけを持つ。** 取得の窓（行に残らない）と、掲載条件で落とした社数（そもそも行にならない）。`build-data.ts` が `companies.meta.filingWindow`・`meta.excluded` に写す
- **`/about` に「単体従業員100人未満で省いた社数」を出す**（1,047社。運営者の指示・2026-08-24）。条件そのものは前から書いてあったが、**数が無いと「有報を出している会社は全部載っている」と読めてしまう**。社数と同じくデータから引く
- **読み方が割れた平均年間給与は2段で絞る**（`unified.resolve_ambiguous_salary`）。①候補のうち「あり得る帯」（`run.plausible_salary_range`＝桁ズレ補正と同じ線）に入るものが1つだけならそれ、②帯で決まらなければ10年推移の中央値に近いほう、③どちらでも決まらなければ落とす。**実測で11社のうち7社が帯で決まり4社が落ちた。** サントリーHD `[1,176万, 176万]`・竹中工務店 `[1,153万, 153万]` は①で決まる。**`fix_salary_typos` より先に走らせること**——あちらは帯の外の値を10の冪で戻すので、先に当てると候補のどれとも一致しない値になる
- **1位が入れ替わった。** 実測値は ヒューリック 2,295万 → Ｍ＆Ａキャピタルパートナーズ 2,266万 → **キーエンス 2,178万（3位）**、35歳そろえは Ｍ＆Ａ 2,330万 → キーエンス 2,178万。**母集団の統計も動く**（実測値 平均719万→693万・sd 200万→199万、35歳 629万→616万・155万→157万）。キーエンスの偏差値は 122.9→124.8（実測値）・150.0→149.5（35歳）
- **`/` の HTML は gzip 63,856 B → 92,797 B**（予算100KBの90.6%）。**余白は 7.6KB。** 1行あたりの情報を増やす変更がそのまま予算にぶつかる（E0・ADR-0013）
- **派生データの充足率をビルドのサマリーに出す**（AC-8）。E2 の時点では 10年推移63.1%・働きやすさ52.3%・稼ぐ力63.0%・ロゴ54.9%。**画面は壊れない**（どれも「掲載なし」を持つ）ので、**数を出さないと気づけない**。追随は E3〜E6
...
- **同じ年なら後ろの年を省く**（`2026年3月期〜4月期`）が、**年が違えば省かない**——`2025年3月期〜5月期` は2025年5月期と読めてしまう。拡大後の実データは `2025年3月期〜2026年5月期`
...
- **Issue #22: 掲載企業数を増やす（E0〜E6 すべて実装済み）。** 拡大そのもの（E2・#173）・初回ロードのペイロード方式（E0・#174）・ロゴ（E3・#175）・10年推移（E4・#176）・働きやすさ（E5・#177）・稼ぐ力（E6・#182）。
  - **E0 が F1（Astro へのカットオーバー・#209）より先に入っているのは意図した順序。** Astro は島の props を HTML の属性に直列化するので `__next_f` より重く、実測で `/` の HTML が gzip +4,024 B（1,867社ぶん）になる。2,961社では E0 前の余白 7.2KB をほぼ食い潰していた（`docs/framework/overview.md`「実施順序」）。**E0 で `/` は gzip 19,860 B になったので、この制約は解けている**——サーバーが渡すのは30行ぶんで、直列化の形式の差は予算に効かない。**F1 を当てた後の実測は gzip 19,420 B**（raw 246,408 B）で、心配していた増加は現れなかった
  - **派生データは全部追随した。** ロゴは E3（#175）で 54.9% → **84.7%**、10年推移は E4（#176）で 63.1% → **100%**、働きやすさは E5（#177）で 52.3% → **80.0%**（W2・#215 の異常値の除去で 79.9%）、稼ぐ力は E6（#182）で 63.0% → **99.9%**
  - **E6 は書類を取り直さなくてよかった。** E4 が10年ぶん26,865件をキャッシュ済みで、`pipeline/performance/extract.py` はそこから読む（`python3 extract.py` を1回回すだけで済んだ）
