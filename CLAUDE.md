...
**掲載企業数の拡大は `expansion` 施策**（`docs/expansion/`・親 Issue #22）。取得の窓を直近12か月に広げて母集団を広げる（ADR-0011）と、全件embed をやめる（ADR-0013）の2つが決まっている。Unit は E0〜E6（Issue #172〜#177・#182）で、**E0・E1・E2・E3・E5 が実装済み**（`docs/expansion/initial-payload/`・`period-range/`・`universe/`。E3・E5 は既存の Unit のデータを作り直しただけなので docs は元の Unit の側にある）。**残るのは E4（10年推移 #176）と E6（稼ぐ力 #182）。****Unit の ID に `P` は使えない**（`performance` 施策が使っている）。
...
- **Issue #22: 掲載企業数を増やす。** 拡大そのもの（E2・#173）・初回ロードのペイロード方式（E0・#174）・ロゴ（E3・#175）・働きやすさ（E5・#177）は実装済みで、**残るのは E4（10年推移 #176）と E6（稼ぐ力 #182）**。
  ...
  - **新しく入った1,094社のうち、10年推移と稼ぐ力がまだ空。** 画面は壊れない（どれも「掲載なし」を持つ）が、充足率がビルドのサマリーに毎回出る。**ロゴは E3（#175）で 54.9% → 84.7%、働きやすさは E5（#177）で 52.3% → 80.0%**
  - **E4 は10年ぶんの書類（27,199件）を取り直すので、取得だけで数時間かかる**（並列3・EDINET の流量制限）。コンテナが落ちても `fetch_history.py` はキャッシュ済みを飛ばすので、そのまま回し直せばよい
