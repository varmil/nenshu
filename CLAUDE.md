...
- **Issue #214: 有報の要約とAI分析（ADR-0015 と C8 は済み・C9/C10 が残り）。** 企業詳細ページに**2つのセクション**を足す——有報の節（MD&A・リスク・課題・人的資本）の**要約**（評価語なし）と、**AI分析**（主観的な評価を積極的に入れ、冒頭に「好調かどうかの一言」）。**精緻化は Issue の本文にある**（決まったこと3つ・例5社・プロンプトに書く材料・実測値）。着手の順は **ADR → C8 調達（#240）→ C9 生成（#241）→ C10 表示（#242）**。
  - **ADR-0015 が ADR-0010 の決定2 を分析セクションに限って上書きした。** 覆るのは**「評価語を書かない」の1点だけ**で、「原文に無い事実を書かない」——モデルの一般知識・報道・世評を材料にしない——は**両セクションで守る**。分析が使ってよい数値は**同じページが既に表示しているもの**だけ（読者が根拠を同じページの中で突き合わせられることが、評価を書いてよい前提）。**読者が2つを見分けられることは要件**で、満たせない見せ方は採らない
