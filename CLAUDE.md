...
**Issue #21（UI改善・Claude Design の `改善案.dc.html`）に着手中。** アートボード 5a/5b/5c（実測値モード）は U11 で実装済み。残りは後続の Unit に分けてある——**U12**（ランキングのレイアウト刷新: PC2カラム＋左サイドバー絞り込み・行の高密度化 #41・年収バー #24・偏差値列・並び替え・業種チップ・ヘッダへの検索統合）、**C2**（企業詳細のレイアウト刷新: 2カラム・要点・水準が近い会社・分布ヒストグラム・年齢別表・この数字の作り方）、**T0**（平均年収の10年推移 #54。EDINET から約18,000書類の遡及取得が要るので C2 の前に着地させる）。計画は `~/.claude/plans/tingly-sleeping-puppy.md`。
