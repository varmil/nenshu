...
**掲載企業数の拡大は `expansion` 施策**（`docs/expansion/`・親 Issue #22）。**Inception だけ済んで未着手。** 取得の窓を直近12か月に広げて母集団を 2,962社にする（ADR-0011）と、全件embed をやめる（ADR-0012）の2つが決まっている。Unit は E0〜E5。**Unit の ID に `P` は使えない**（`performance` 施策が使っている）。
