...
**モックに合わせないと決めたものがある**（U13 の `docs/ranking/ranking-mock-alignment/design.md` に対照表）。偏差値には上位◯%を併記する（モックは数字だけ）・年収バーはページ内正規化（モックは上限2,500万円）・実測値でも年齢スイッチを残す（モバイルのモックは消している）・会社の紹介文は作らない（データが無い）。

**ランキングは U12（`docs/ranking/ranking-refresh/`、Issue #80）でレイアウトを刷新し、U13（`docs/ranking/ranking-mock-alignment/`、Issue #88）でモックに合わせ直した。**

- **表は4列**（順位 / 会社名・業種 / 金額＋バー / 偏差値）。平均年齢・在籍年数・従業員数は社名の下の meta 行にある（`CompanyMetaLine`）。**U12 の7列では社名が省略記号で切れていた**（「三菱商事…」）。`truncate` は `block` にしないとインラインのまま溢れる（360px で横スクロールが出た）
