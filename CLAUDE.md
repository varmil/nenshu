...
順序: C0（#51）→ C1（#52）→ U11（#71）→ **U12（#80）** → C2 → U8（#53）。**U8 のリンクハブは U12 の業種チップで賄えたので、U8 の範囲は canonical・sitemap・robots に狭めた**（`docs/ranking/overview.md`）。

**Issue #21（UI改善・Claude Design の `改善案.dc.html`）に着手中。** アートボード 5a/5b/5c（実測値モード）は U11、4a/2a/5c（レイアウト刷新）は **U12（Issue #80）で実装済み**。残りは、**C2**（企業詳細のレイアウト刷新: 2カラム・要点・水準が近い会社・分布ヒストグラム・年齢別表・この数字の作り方）だけになった（**T0** 平均年収の10年推移 #54 も実装済み）。計画は `~/.claude/plans/tingly-sleeping-puppy.md`。
