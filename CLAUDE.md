...
**働きやすさ指標は `worklife` 施策**（`docs/worklife/`）。厚生労働省「女性の活躍推進企業データベース」から**平均残業時間・年次有給休暇の取得率・男女の賃金の差異**を取り込み、企業詳細ページに出す。**W0（取り込み・Issue #149）・W1（表示・Issue #150）・W2（異常値の除去・Issue #215）とも実装済み**（`docs/worklife/data-ingest/`・`company-section/`・`outlier-handling/`）。親 Issue #148。**E5（#177）で母集団の拡大に追随させた**（1,548社 → 2,369社 = 79.9%。W2 で 2,367社）。
