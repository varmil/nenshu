...
**レーダーチャート「公開資料による全体像」は P1**（`docs/performance/company-radar/`・Issue #167）。企業詳細ページの先頭に5軸（平均年収・有給の取得・定着（在籍）・稼ぐ力・残業の少なさ）を出す。**男女の賃金の差異は軸にしない**（#154。数値は W1 の節に残る）。
...
- **軸ラベルは5軸とも中央寄せ。** 左右の軸を `start`/`end` にすると「残業の少なさ」が viewBox の外へ出て切れる。中央寄せなら余白が半分で済み、そのぶん図を大きく取れる
...
- **指標リストの並びは 年収 → 残業 → 有給 → 定着 → 稼ぐ力**（`RADAR_LIST_ORDER`。図の時計回りとは別）。**稼ぐ力だけが2行ぶんの高さを持つ**ので末尾に置く。値は `min-w-[4.75rem]` で右寄せ・順位は `w-[5.25rem]` で左寄せに固定し、**掲載なしの行も同じ幅の空きを残す**（詰めるとその行だけ右へ寄る）。業種中央値は値の下に右寄せ
