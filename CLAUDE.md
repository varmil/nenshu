...
**働きやすさ指標は `worklife` 施策**（`docs/worklife/`）。厚生労働省「女性の活躍推進企業データベース」から**平均残業時間・年次有給休暇の取得率・男女の賃金の差異**を取り込み、企業詳細ページに出す。**W0（取り込み・Issue #149）→ W1（表示・Issue #150）。親 Issue #148。** Inception のドキュメントのみ作成済みで、実装は未着手。
...
- **実測（`4e6b3d07-99_20260820_utf8_bom.zip`）: 64,879行・236列、突合1,679社（89.9%）。** 記入率は賃金の差異87.2%・残業61.5%・有給61.2%・いずれか91.4%
...
- **`pipeline/salary35/` は `pipeline/salary/` に改名する**（W0 に含める）。35歳が既定だった頃の名前で、ADR-0007 以降は実態と合っていない。**ディレクトリは「作るデータセット」で切り、ソースはファイル名で表す**（`salary/` は EDINET と e-Stat の2ソースを使うので、ソース名では切れない）
