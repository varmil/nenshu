...
- **モバイルの行は4カラム**（順位 / ロゴ / 社名＋meta / 金額ブロック。アートボード 2a、Issue #119）。**年収バーは金額ブロックの中**で、幅は `w-20`＝80px＝最大値「2,178万円」の実測幅（バーの左端が数値の左端に揃う）。5c の頃は社名・meta・バーを右の1列に縦積みしており、**バーだけが行の全幅に伸びてどの社名の帯か読めなかった**。順位は行の縦中央で `text-xs`。社名は1行で切る。**縮むのは社名と平均年齢だけ**——金額と偏差値は `shrink-0` で360pxでも全部見える。**「本社のみ」バッジと従業員数はモバイルの行に出さない**（PCの表には両方ある）。行の高さはロゴの48pxが決めるので社名の長さによらず等間隔
