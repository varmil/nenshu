...
**企業ロゴは `logo` 施策**（`docs/logo/`・親 Issue #23・ADR-0008）。**L0（調達、#109）・L1（器と表示、#110）とも実装済み。** **2,961社中2,507社（84.7%）にロゴが出る**（E3・#175 で母集団の拡大に追随させ、W2 と同じ日の #221 で2社が頭文字に戻った）。
...
- 取得の失敗はキャッシュしないので、`npm run build:logos` を**走らせ直すたびに数十社ぶん回収される**（429と一時的な失敗）。**ただし回収は先細る**——E3 では1巡目（1,336社）で +875社、2巡目（461社）で +9社だった。到達率 2,507/2,961 = 84.7%、画像は合計13.13MB（1枚平均5.3KB・最大18.4KB）、`logos.json` は gzip 63.6KB
...
- **ロゴの無い454社**（E3 の後、#221 で2社が加わった）。過半が「公式サイトURLが無い」で、Wikidata にも gBizINFO にもURLが無い会社。**届く手段は現状無い。**
