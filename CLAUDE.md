...
**働きやすさ指標は `worklife` 施策**（`docs/worklife/`）。厚生労働省「女性の活躍推進企業データベース」から**平均残業時間・年次有給休暇の取得率・男女の賃金の差異**を取り込み、企業詳細ページに出す。**W0（取り込み・Issue #149）・W1（表示・Issue #150）とも実装済み**（`docs/worklife/data-ingest/`・`docs/worklife/company-section/`）。親 Issue #148。**E5（#177）で母集団の拡大に追随させた**（1,548社 → 2,369社 = 80.0%）。
...
- **`worklife_2026.csv` は2,369行**（突合2,641社のうち、3指標を1つも持たない272社は行を作らない）。`worklife.json` は gzip **186.2KB** で上限220KB。**上限を超えたとき注釈を切り出すかは、Worker バンドルが 3MiB に近づいたかで決める**（`docs/worklife/overview.md`）——E5 の実測でバンドルは gzip 2.089MB（66.4%）なので同梱のまま
...
- **Issue #22: 掲載企業数を増やす。** 拡大そのもの（E2・#173）・初回ロードのペイロード方式（E0・#174）・ロゴ（E3・#175）・10年推移（E4・#176）・働きやすさ（E5・#177）は実装済みで、**残るのは E6（稼ぐ力 #182）だけ**。
  ...
  - **新しく入った1,094社のうち、稼ぐ力だけがまだ空。** 画面は壊れない（「掲載なし」を持つ）が、充足率がビルドのサマリーに毎回出る。**ロゴは E3（#175）で 54.9% → 84.7%、10年推移は E4（#176）で 63.1% → 100%、働きやすさは E5（#177）で 52.3% → 80.0%**
