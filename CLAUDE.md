...
- **モバイルの行は「順位・ロゴ」と「右の列」の2つに割れている**（アートボード 5c）。社名＋金額 / meta＋偏差値 / 年収バー の3段はすべて右の列に積む——**バーを行の全幅に伸ばすとどの社名の帯か読めない**。順位は行の縦中央。社名は1行で切り、「本社のみ」は meta 行に置く
...
- トップページの HTML は gzip 72,232 B → **71,671 B**。`/company/[id]` は 12,873 B → **13,283 B**（共通ヘッダに検索ボタンが増えたぶん）。その後 Issue #103 で1ページ30件にして **70,283 B → 62,563 B**（同一手法で `next start` に対して再計測した前後。`/company/[id]` は変わらない）
...
- **行の中の強弱は 社名（太字）＞ 順位（18px）＞ 金額（16px）**（Issue #96）。金額を20pxにすると社名より先に目に入り、「どの会社の数字か」を読む順序が逆になる
