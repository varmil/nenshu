...
- **英字社名からドメインを推定して公式サイトを見つける方法は使わない**（L2・#243）。正解のある500社で測って **precision 0.908 [0.849, 0.945]**。**同名の別会社が切り分けられない**のが理由で、検証を厳しくしても recall が落ちるだけで precision は上がらない（届かせるには法人番号や所在地の一致が要るが、それらは会社概要のページにしか無い）。**測る道具（`guess.ts`・`measure-site-guess.ts`）は残してあるが、`build-logos.ts` の経路には入っていない**
- **測定の副産物: Wikidata の P856 は当てにならないことがある。** アステラス製薬は `web.archive.org`、ナイルはブログのURL、芝浦機械は社名変更前の `toshiba-machine.co.jp` が登録されていた
...
- **ロゴの無い425社。** **271社は公式サイトのURLがどこにも無く**（Wikidata にも gBizINFO にも）、残りはサイトに届かない（403・503・接続不可）。**英文社名からドメインを推定する案は L2（#243）で測って却下した**——precision 0.908 [0.849, 0.945] で条件の 0.99 に届かず、**残る誤りは「同名の別会社」で社名の一致では切り分けられない**（やまびこ → やまびこの森、アルペン → アルペンチーズケーキ）。`docs/logo/site-guess/design.md`。
- **Issue #55: 株価・信用格付け。** J-Quants・edinetdb.jp からの調達可否を確認するところから。**再配布可否の判断は運営者が行う**（product.mdの制約に明記）。
- **Issue #214: 有報の要約とAI分析（Inception 済み・未着手）。** 企業詳細ページに**2つのセクション**を足す——有報の節（MD&A・リスク・課題・人的資本）の**要約**（評価語なし）と、**AI分析**（主観的な評価を積極的に入れ、冒頭に「好調かどうかの一言」）。**精緻化は Issue の本文にある**（決まったこと3つ・例5社・プロンプトに書く材料・実測値・Unit 案）。着手の順は **ADR → C8 調達 → C9 生成 → C10 表示**で、**ADR が先**（ADR-0010 の決定2 を分析セクションに限って上書きする）。
  - **原文は取り直さなくてよい。** 必要な4節は全社100%取れることを無作為60社で実測済みで、書類はキャッシュ（`pipeline/salary/cache/`・2,961件）にある。`pipeline/salary/edinet.py` の `TEXT_BLOCKS` に要素を足すだけ
  - **量が C6 の約20倍**（1社あたり 736字 → 約14,400字）。C6 が12回のセッションだったので単純計算で約96回。**パイロットで原文の切り方を決めてから回す**
