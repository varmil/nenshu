...
順序: C0（#51）→ C1（#52）→ U11（#71）→ U12（#80）→ C2（#83）→ **U13（#88）** → **C3（#89）** → **U8（#53）** → **U14（#121）** → **U15（#132）**。**U8 のリンクハブは U12 の業種チップで賄えたので、U8 の範囲は canonical・sitemap・robots に狭めた**（`docs/ranking/overview.md`）。**U8 は実装済み**（`docs/ranking/search-discovery/`）。
...
- **表は4列**（順位 / 会社名 / 金額＋バー / 偏差値）。平均年齢・在籍年数・従業員数は社名の下の meta 行にある（`CompanyMetaLine`）。**業種は meta 行に出さない**（公開後の指摘で外した。行が長くなって末尾が見切れていた）。**U12 の7列では社名が省略記号で切れていた**（「三菱商事…」）。`truncate` は `block` にしないとインラインのまま溢れる（360px で横スクロールが出た）
...
- **モバイルの行は4カラム**（順位 / ロゴ / 社名＋meta / 金額ブロック。アートボード 2a、Issue #119）。**年収バーは金額ブロックの中**で、金額は**左寄せ**でバーと左端が揃う。**幅は `w-20`＝80px**（4桁で金額とバーの幅が揃う値）。**`whitespace-nowrap` を外さないこと**——webfont を持たずOSのフォントで組むので幅が環境で変わり、`nowrap` の無い80pxでは実機で「円」だけが2行目に落ちた。`nowrap` があれば2行にはならず、収まらない環境では右へわずかにはみ出す（`p-4` の16pxに収まる範囲）。バーの幅は全行で同じでないと長さを見比べられないので、`w-fit` で逃げられない。**バーはモバイルだけ3px**（`SalaryBar` の `size="row"`。PCの表は6pxのまま）——88pxの器で6pxだとバーが金額より強く目に入る。5c の頃は社名・meta・バーを右の1列に縦積みしており、**バーだけが行の全幅に伸びてどの社名の帯か読めなかった**。順位は行の縦中央で `text-xs`。社名は1行で切る。**縮むのは社名と平均年齢だけ**——金額と偏差値は `shrink-0` で360pxでも全部見える。**「本社のみ」バッジと従業員数はモバイルの行に出さない**（PCの表には両方ある）。行の高さはロゴの48pxが決めるので社名の長さによらず等間隔
...
- トップページの HTML は gzip 72,232 B → **71,671 B**。`/company/[id]` は 12,873 B → **13,283 B**（共通ヘッダに検索ボタンが増えたぶん）。その後 Issue #103 で1ページ30件にして **70,283 B → 62,563 B**（同一手法で `next start` に対して再計測した前後。`/company/[id]` は変わらない）。Issue #119 のモバイル行で raw 371,849 B → **355,341 B**（−4.4%）。**gzip 後の値は同じビルドでも `next start` の起動ごとに揺れるので、前後を比べるときは raw で見る**
...
- **行の中の強弱は 社名（太字）＞ 順位 ＞ 金額（16px）**（Issue #96）。金額を20pxにすると社名より先に目に入り、「どの会社の数字か」を読む順序が逆になる。**順位は PC が18px・モバイルが12px**（Issue #119。狭い行では18pxだと社名と同じ強さで目に入る）
