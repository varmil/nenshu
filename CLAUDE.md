...
**掲載企業数の拡大は `expansion` 施策**（`docs/expansion/`・親 Issue #22）。取得の窓を直近12か月に広げて母集団を広げる（ADR-0011）と、全件embed をやめる（ADR-0013）の2つが決まっている。Unit は E0〜E6（Issue #172〜#177・#182）で、**E1・E2 が実装済み**（`docs/expansion/period-range/`・`docs/expansion/universe/`）。**Unit の ID に `P` は使えない**（`performance` 施策が使っている）。
...
- **Issue #22: 掲載企業数を増やす。** 拡大そのもの（E2・#173）は実装済みで、**残るのは派生データの追随**（E3 ロゴ #175・E4 10年推移 #176・E5 働きやすさ #177・E6 稼ぐ力 #182）と、初回ロードのペイロード方式（E0・#174）。**E0 は Astro への移行（F1・#209）より先に入れること**——Astro は島の props を HTML の属性に直列化するので `__next_f` より重く、実測で `/` の HTML が gzip +4,024 B（1,867社ぶん）になる。2,961社では **いまの余白 7.2KB をほぼ食い潰す**ので、E0 が無いと F1 単体で予算100KBに触りかねない（`docs/framework/overview.md`「実施順序」）。
  - **新しく入った1,094社のうち、10年推移・働きやすさ指標・稼ぐ力がまだ空。** 画面は壊れない（どれも「掲載なし」を持つ）が、充足率がビルドのサマリーに毎回出る。**ロゴは E3（#175）で追随済み**（54.9% → 84.7%）
