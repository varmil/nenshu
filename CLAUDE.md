...
**Workers 無料枠の CPU は 10ms/リクエストで、実際に超えていた**（`runtime` 施策・`docs/runtime/`・親 Issue #118・R0 は Issue #165 で実装済み）。踏んでいたのはクローラで、1,867社ぶんの異なるURLを世界中のコロから叩くのでエッジキャッシュは全部ミスし、isolate は毎回冷えている。

- **`import` したデータファイルは、その画面が1バイトしか使わなくても丸ごと `JSON.parse` される。しかも isolate の初回リクエストに課金される。** どのページが何を読んでよいかは `docs/runtime/spec.md` 2.1 の表が正で、**違反は lint で落ちる**（ADR-0011）。`/` は `stats.json` 131KB を読んで 337B だけ、`logos.json` 202KB を読んで「ロゴがあるか」だけを使っていた
- **切り出しは生成側（`pipeline/`）でやる。** web 側で切り出しても、丸ごと読んだあとで捨てることになり CPU は1msも減らない。`population.json`（337B・`build-data.ts`）と `logo-ids.json`（11.3KB・`build-logos.ts`）がそれ。**`logo-ids.json` は「IDの配列」で、`companies.rows` と同じ並びのマスクではない**——マスクにすると生成が循環する
- **`buildCompanyView` は React の `cache()` で1リクエスト1回。** `generateMetadata` と本体が同じ会社の同じ数字を出すのに2回走っていた（0.455〜0.689ms/回）
- **`findNeighbors` は isolate ごとの索引を引く。** 9基準ぶん呼ばれるので、素直に書くと1リクエストで延べ33,606行を走査していた。**業種カーブは「カーブ名」で引く**——`curves.curves` のキーの並びは `companies.curveKeys` と別でありうる
- **効果は機構の単位で測る。** `wrangler dev --local` の cold は 508〜1,776ms（4.7MB の `handler.mjs` を毎回一から評価するため）で、数msの差は絶対に見えない。`JSON.parse` はファイル単位で、自分のコードは**同じプロセスで新旧を並べて**測る（最初の1本は JIT を含むので捨てる。捨てずに同じ関数が 0.885ms と 0.263ms を出した）。`wrangler dev` の CPU は `/proc/<workerd>/schedstat` の第1フィールド（ns）で測れる——`stat` の `utime` は10ms刻みで粗すぎる
- **新しいデータファイルを足す Unit は必ずこの問いを通る。** C5〜C7（#159〜#161）も同じ
- **足りなければ次は `/company/[id]` の静的生成**（ADR-0004 の追記になる）。`stats.json` の入れ子を平らにする案は却下した（0.37ms のために「行がずれると別の会社の順位を出す」表現へ移す取引が見合わない）
...
- **バーは10メモリ・1メモリ＝10時間/10%で上限100。値そのものは丸めない**（有給103.0 はそのまま）。**塗られていないメモリは描かない**——空セルを敷くと棒の短さより器の長さが先に目に入る。**賃金の差異にバーは描かない**（100を基準に上下する値で「長いほど良い」の向きが揃わない）
...
- **トップページの HTML は raw 373,821 B のまま±0**（`app/page.tsx` から読まない）。`/company/6861` は 97,889 B → 103,281 B、区分4件＋注釈のある `/company/8058` は 110,254 B
...
**稼ぐ力（一人当たり経常利益）は `performance` 施策**（`docs/performance/`）。**レーダーチャートの軸を決める親 Issue #154 が「賃金差を外し、稼ぐ力を入れる」と決めた**ので、その軸のためのデータを作る施策として新しく立てた（`market-data`（#55）に寄せない理由は intent.md）。**P0（取り込み・Issue #155）・P1（レーダー表示・Issue #167）は実装済み**（`docs/performance/profit-per-employee/`・`docs/performance/company-radar/`）。**P2（稼ぐ力の推移・Issue #168）は未着手。**
...
- **実測: 1,864社（99.8%）。5期そろうのが1,855社、赤字（中央値が0以下）が59社、連結の従業員数が無くて単体で代用したのが189社。** 銀行業82社・保険業13社・その他金融業33社とも欠損なし（営業利益が無いことを理由に欠ける会社は出ない）
- **赤字は負のまま持つ。** 捨てると「データが無い」と区別できなくなる
- **業種差が19倍ある**（海運業2,524万円・輸送用機器131万円）ので**業種中央値を数字として併記する**。**平均ではなく中央値**——電気機器はキーエンスが業種中央値の22.4倍で、平均だと業種の代表値がキーエンスに引きずられる
...
- **母集団は軸ごとに違う**（平均年収・定着 1,867社 / 稼ぐ力 1,864社 / 有給 895社 / 残業 974社）。**欠測を最下位として数えない**——掲載率6割の軸で、公表している会社が軒並み上位に寄る
...
- **`/` は母集団統計（`count`/`bases`/`population`）だけを渡す。** `rankAll`/`rankIndustry` は1,867×9の配列2本で、クライアントに直列化するとページの予算を超える。渡す結果は1KB未満（`population.test.ts` が固定）。**読む側も `stats.json` ではなく `population.json`（337B）にした**（R0・#165）——渡す量だけ絞っても、読む量が減らなければ isolate の初回リクエストの CPU は減らない
...
- **`/` に `logos.json` を丸ごと渡さない**（gzip 41.5KB）。渡すのは**ロゴの有無だけ**——`companies.rows` と同じ並びの1,867文字の文字列（gzip 約250B）。**縦横比は渡さない**——器の寸法が固定で中身は `object-contain` なので、画像が届いてもレイアウトは動かない。企業詳細ページはマスクではなくその画面に出る46社ぶんのIDだけを渡す。**読む側も `logos.json` ではなく `logo-ids.json` にした**（R0・#165）。`logos.json` を読んでよいのは `/about` だけで、他は lint が止める
