...
- **`import` したデータファイルは、その画面が1バイトしか使わなくても丸ごと `JSON.parse` される。しかも isolate の初回リクエストに課金される。** どのページが何を読んでよいかは `docs/runtime/spec.md` 2.1 の表が正で、**違反は lint で落ちる**（ADR-0011）。`/` は `stats.json` 131KB を読んで 337B だけ、`logos.json` 204KB を読んで「ロゴがあるか」だけを使っていた
- **切り出しは生成側（`pipeline/`）でやる。** web 側で切り出しても、丸ごと読んだあとで捨てることになり CPU は1msも減らない。`population.json`（337B・`build-data.ts`）と `logo-ids.json`（11.6KB・`build-logos.ts`）がそれ。**`logo-ids.json` は「IDの配列」で、`companies.rows` と同じ並びのマスクではない**——マスクにすると生成が循環する
...
- **新しいデータファイルを足す Unit は必ずこの問いを通る。** W1（#150）の `worklife.json` は raw 491KB・1.734ms で、丸ごと読むと `/company/[id]` の cold が 2.72ms から 4.46ms に戻る。C5〜C7（#159〜#161）も同じ
...
- **`/` に `logos.json` を丸ごと渡さない**（gzip 41.7KB）。渡すのは**ロゴの有無だけ**——`companies.rows` と同じ並びの1,867文字の文字列（gzip 約250B）。**縦横比は渡さない**——器の寸法が固定で中身は `object-contain` なので、画像が届いてもレイアウトは動かない。企業詳細ページはマスクではなくその画面に出る46社ぶんのIDだけを渡す。**読む側も `logos.json` ではなく `logo-ids.json`（11.6KB）にした**（R0・#165）。`logos.json` を読んでよいのは `/about` だけで、他は lint が止める
