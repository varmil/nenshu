...
値は main が `042f508`（W1・P0・P1 と R1 の事前生成まで入った時点）のもの。

| ページ | main | S4 | 差 |
| --- | ---: | ---: | ---: |
| `/` raw | 373,821 B | 374,617 B | +796 |
| `/` gzip | 63,665 B | 63,855 B | +190 |
| `/company/[id]` raw | 111,952 B | 112,748 B | +796 |

**main が4回進んでも差は +796 B のまま**だった（R0 の投入・R0 のリバート・W1 の
追加・R1 の事前生成をまたいで測り直している）。`/company/[id]` の絶対値だけが動いて
いるが、これは W1 が働きやすさの節を足したためで、S4 とは関係が無い。
