...
`wrangler dev --local` に対して40リクエストを投げ、`/proc/<workerd>/schedstat` の第1フィールド（ns）の差分を40で割った値。

**「前」は同じコードの `force-static` を `force-dynamic` に入れ替えただけのビルドで測った。** 同じデータ・同じ節（W1・P0・P1 を含む）・同じセッションで、違うのは描くか引くかだけになる。`/about` はどちらのビルドでも事前生成されている（`incrementalCache` は両方に入っている）ので、対照として使える——**動いていないことが、測っているのが描画のぶんであることの証拠になる。**

| | 前 | 後 | 床を引いた「アプリぶん」 |
| --- | ---: | ---: | --- |
| `/company/8282` | 29.6〜29.7ms | **14.0〜14.1ms** | 24ms → 8〜9ms |
| `/company/7203` | 26.5〜29.0ms | **13.6〜13.7ms** | 21〜23ms → 8ms |
| `/about`（`incrementalCache` の前） | 47.7〜59.6ms | **12.4〜13.5ms** | 42〜54ms → 7〜8ms |
| `/`（動的のまま） | 41.7〜46.5ms | 46.3〜48.5ms | 変わらない |
| `/favicon.ico`（床） | 5.3〜6.1ms | 5.1〜5.6ms | |
...
| | 前 | 後 |
| --- | ---: | ---: |
| `.open-next/assets` のファイル数 | 1,655 | **3,528** |
| 同 容量 | 15MB | **344MB**（うち `cdn-cgi/_next_cache` が 330MB） |
...
**`enableCacheInterception` は使わない**（Issue 183・PR #184）。入れて本番へ出したところ、**ブラウザから `/about?_rsc=…` が毎秒およそ128回・15秒で1,918回**飛んだ。

Next.js のルーティングに入る前にキャッシュから返す設定なので、**プリフェッチのリクエスト（`Next-Router-Prefetch: 1`）にもフルの RSC ペイロードを返してしまう。** クライアントのルーターは頼んだプリフェッチ結果が返っていないと判断して取り直し、それが延々と続く。ヘッダの「計算方法」リンクが全ページにあるので、どのページを開いても起きた。

同じビルドでこの設定だけを入れ替えると `/about?_rsc=…` は **1,918回 → 2回**（正常なプリフェッチ）になる。

**稼げていたのは 19.7ms → 15.6ms。** 事前生成そのもの（21〜24ms → 8〜9ms）に対する上積みでしかなく、**Workers 無料枠の10万リクエスト/日を読者1人が13分で使い切る**壊れ方と引き換えにしてよいものではなかった。

### なぜ E2E で捕まらなかったか

**プリフェッチは本番ビルドでしか動かない。** E2E は dev サーバーに対して走るので、311件すべて通っていた。`npm run measure:prefetch` も `next start`（Node）に対して `/company/` の先読みだけを数えており、OpenNext の設定に由来するこの壊れ方は原理的に拾えない。

**兆候は出ていた。** R1 の検証中、Worker 相手の E2E で `/about?_rsc=…` が20件記録されるのを見て「本番ビルドのプリフェッチだから正常」と片づけている。1つのリンクに20件は多すぎで、そこで止まるべきだった。

`e2e/prefetch-loop.spec.ts` を置いた。ページを開いて8秒放置し、**同じURL（`_rsc` の値は畳む）が15回を超えたら落ちる。** 壊れたビルドで実際に落ちること（1,422〜1,951回を検出）と、直ったビルドで通ることの両方を確かめてある。
