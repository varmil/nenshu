...
`import` したファイルは、その画面が中身を1バイトしか使わなくても丸ごと `JSON.parse` される。しかも isolate の初回リクエストに課金される。`/` は `stats.json` 130KB を読んで 337B ぶんだけ使い、`logos.json` 202KB を読んで「ロゴがあるか」だけを使っていた。
