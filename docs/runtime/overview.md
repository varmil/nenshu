...
**`worklife.json`（491KB・parse 1.568ms）は W1（Issue #150）が `/company/[id]` に足す。** spec 2.1 の表はそれを見越して「W1 以降は読んでよい」と書いてある。**ただし当該1社ぶんしか使わないので、W1 は R0 と同じ問いを一度通ること**——1社ぶんのために全社ぶんを読むなら、`logo-ids.json` と同じ切り出しが要る。
