...
`h1` はブランドを含まず、ページの内容だけを表す（`docs/site-chrome/spec.md`）。文言は `rankingHeading()`（`lib/seo/ranking.ts`）の1か所から出る。

| 画面の状態 | `h1` | title |
| --- | --- | --- |
| `/` | `平均年収ランキング` | `OpenReport \| 有価証券報告書ベースの平均年収ランキング {社数}社【{決算期}】` |
| `/?age=35` | `35歳年収ランキング` | `35歳年収ランキング \| OpenReport` |
| `/?ind=銀行業` | `銀行業の平均年収ランキング` | `銀行業の平均年収ランキング \| OpenReport` |
| `/?age=35&ind=銀行業` | `銀行業の35歳年収ランキング` | `銀行業の平均年収ランキング \| OpenReport`（寄せ先） |
| `/?ind=銀行業&emp=1000-` | `銀行業の平均年収ランキング` | `/` の title（寄せ先） |
| `/?ind=存在しない業種` | `平均年収ランキング` | `/` の title（寄せ先） |

- **ファセットの title は `h1` にブランドを足したもの。** title も `rankingHeading()` から組み立てる——別々に書くと、片方だけ言い回しを変えたときにインデックスさせるページの `h1` と title が食い違っても気づけない。業種つきの `h1` は 2026-09-26 に足した。それまでは業種で絞っても `平均年収ランキング` のままで、`/?ind=X` が33件あるのに `h1` は1種類しか無かった
- **`h1` は画面の状態を、title は寄せ先を表す。** 非正規URLの title は寄せ先のものを返す（`rankingPageMeta` の決め。og:url と og:title を食い違わせないため）。一方で `h1` は読者が見ている表の見出しなので、`?age=35&ind=銀行業` では銀行業の35歳そろえを名乗る。検索エンジンが評価するのは寄せ先の `/?ind=銀行業` で、そちらでは `h1` と title がそろっている
- **寄せる絞り込み（`emp`・`ten`・`aage`・`q`・`sort`）は `h1` に入れない。** 表の上の「適用中」のチップが持っている。業種だけを入れるのは、インデックスさせる軸だから
- **33業種に無い `ind` は `h1` に出さない。** そのまま載せると、任意の文字列を `h1` に持つページをURLだけで作れる
- **直下のリード文の社数も `h1` と同じ範囲を数える**（`features/ranking/lib/lead.ts` の `rankingLead`）。業種で絞っていれば `…（単体）で比べた銀行業の82社。`、そうでなければ `…で比べた2,961社。`。`h1` が `銀行業の平均年収ランキング` なのに直下が `2,961社` だと、銀行業が2,961社あるように読める。業種の社数は母集団の内訳なので、ほかの絞り込みでは数え直さない（`/` の掲載社数と同じ扱い）。**動詞は「比べた」で、「並べた」にしない**——平均年齢順・従業員数順に並び替えてもリード文は変わらないので、金額で並べたと書くとその画面で事実と合わなくなる。長い業種名ではモバイルで3行になる（360px では業種が無くても年齢そろえで3行）。4行にならないことを E2E が見ている
- **リード文にはどちらの表示基準でも「有価証券報告書」を入れる。** 年齢そろえは `…の有価証券報告書の平均年間給与を、業種の賃金カーブで35歳時点に補正した2,961社。`。金額の出どころを示す語で、title に入れない代わりに description と本文で名乗る（上の「タイトルに入れるのは `/` だけ」と同じ理由）。以前の年齢そろえは、この語を PC でだけ足す2文目（「元になる金額は有価証券報告書の平均年間給与です。」・`hidden md:inline`）に置いていて、**モバイルの画面には1つも出ていなかった**。1文目に入れたので2文目は外した。E2E は `innerText` で見る——`textContent` では隠れた字も拾うので、2文目に戻しても通ってしまう
- **業種つきの `h1` は「◯◯の」の後ろでだけ折り返す。** 390px の本文幅は 20px の字で17字ぶんしか無く、`ガラス・土石製品の平均年収ランキング`（18字）は1行に収まらない。何もしないと `…ランキン` / `グ` のように語の途中で切れる（実測）。`break-keep` で字の間の改行を止め、`<wbr>` で境目にだけ改行の機会を置く。断片は最長でも `証券、商品先物取引業の`（11字）なので、`break-keep` で1行からはみ出すことはない。E2E は最長の `証券、商品先物取引業の60歳年収ランキング` を 390 / 360px で見る
...
### `preview_urls` の変更は main に入れるまで効かない

**Worker の設定は本番デプロイでしか更新されない。** `preview_urls: true` をブランチに入れた状態で3回デプロイしてもプレビューURLは出ず、**その変更を main にマージした直後のビルドで出た**（2026-08-21・Issue #119・PR #124 → #127）。

観測した並び。

| 出来事 | 説明 |
| --- | --- |
| ずっと出ない | `preview_urls` 未指定 → 既定で `workers_dev`（＝`false`）に一致 |
| ダッシュボードで有効にした直後の1回だけ出る | 設定が生きていた1回。**そのデプロイ自体が `false` を書き戻す** |
| `preview_urls: true` をブランチに入れても出ない | ブランチのビルドでは Worker の設定が更新されない |
| main にマージしたら出た | 本番デプロイで設定が反映された |

**「ダッシュボードで直す → 次のデプロイで戻る」を繰り返しているときは、まず `wrangler.jsonc` に書いて main に入れる。** ブランチで試して「効かない」と判断すると、原因を設定ファイルの外に探しに行くことになる（実際にそうなった）。
