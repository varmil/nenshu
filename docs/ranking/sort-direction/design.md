...
| `age` | `desc` | `asc` | 平均年齢 高い順 ／ 平均年齢 若い順 |
| `employees` | `desc` | `asc` | 従業員数 多い順 ／ 従業員数 少ない順 |

`features/ranking/lib/sort.ts` の `SORT_DEFAULT_ORDER`・`SORT_AXIS_LABEL`・`SORT_DIRECTION_LABEL` が正。

**既定の向きは3軸とも `desc`＝大きい順に揃えてある。** U15 の最初の実装では `age` だけ `asc`（若い順）にしていた——U15 より前の3通りがそうで、外に出ている `?sort=age` の意味を変えたくなかったため——が、**1つの軸だけ向きが違うと、チップを押したときに何が起きるかを軸ごとに覚えることになる**（運営者の指摘）。矢印もその軸だけ上を向く。揃えたことで規則は「押すとまず大きいほうから並ぶ。もう一度押すと逆」の1行になった。

**代償として、U15 の直後に配った `?sort=age`（若い順のつもり）は高い順で開く。** sitemap には載せておらず canonical も `/` へ寄せているので検索インデックスには無い。影響を受けるのは共有されたリンクだけで、その数より規則が1行で済むことを採った。

向きの言い方は軸ごとに違う（「昇順／降順」とは書かない）。**`age` の `desc` は「高い順」で、`salary` の `desc`（高い順）と字面が同じ**になるが、軸の名前と必ず並べて出すので取り違えようがない。
...
**既定の向きなら軸の名前だけ、逆向きなら向きを足す。** 3軸とも既定が降順に揃ったので、**書き出す綴りに `-desc` は現れない**。それでも `splitSortParam` は `-desc` を読める——書く側が使わない綴りでも、読む側が落とすと `?sort=age-desc` が既定の並びに倒れてしまう（`urlState.test.ts` が固定）。

向きを必ず書く形（`age-desc`）にもできるが、それだと `?sort=age` が「正規形ではないURL」になり、同じ並びに2つの綴りが残り続ける。軸のトークン（`salary`・`age`・`emp`）に `-` は含まれないので、読む側は末尾だけ見れば割れる。
...
- **矢印は向きを表す**（`ChevronDownIcon` = 大きい順、`ChevronUpIcon` = 小さい順）。U13 までは常に下向きで、「平均年齢が若い順」でも下を向いていた。**モバイルでは向きの語を出さないので、矢印が唯一の手がかりになる。** 既定を3軸とも降順に揃えたので、**既定の状態ではどのチップも下向き**になる
