...
| | 現状（1,867社） | 2,962社の見込み | 無料枠の上限 |
| --- | ---: | ---: | --- |
| ページ数 | 1,874 | 2,969 | — |
| アセット | 344MB | **545MB** | 1ファイル 25MiB |
| ファイル数 | 3,528 | **約4,600** | 20,000 |
| 事前生成の時間 | 19.9秒 | 32秒 | — |

**見込みは比例で伸ばした値で、実測ではない。** ページ数がそのまま増え、1ページあたりの大きさは変わらないので比例で読んでよい（gzip のときと違う）。**上限には当たらないが、E2 の後で実測して ADR-0012 の数字を更新する。**
