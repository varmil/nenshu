...
**Next.js（App Router）を `output: 'export'` で使い、UI は shadcn/ui + Tailwind CSS で組む。TypeScript。バージョンは特定のメジャーバージョンに固定せず、着手時点の最新安定版を使う。**

運営者は1名で長期の保守工数が薄いため、ライブラリは Next.js に限らず全般的に、固定より最新追従を優先する。セキュリティ修正・エコシステムの実装例（AIエージェントが参照する情報量）の両方が最新版に集まるため。特定バージョンへの固定が必要な理由（破壊的変更の回避など）が生じたときだけ、その理由をここに追記する。
