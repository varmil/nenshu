...
> **2026-08-24 追記（[ADR-0012](0012-prerender-company-pages.md)）**: 「全ページをリクエストごとに描画する」は `/company/[id]`・`/about`・`/robots.txt`・`/sitemap.xml` について置き換わった。これらはビルド時に生成し、Worker は結果を引いて返すだけにする（Workers 無料枠の CPU 10ms を超えていたため。Issue #118）。**`/` は `searchParams` を読むので動的なまま。** キャッシュヘッダの規則（`web/lib/cache/headers.ts`）とエッジキャッシュの扱いは変わらない。
