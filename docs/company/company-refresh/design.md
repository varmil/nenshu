...
**目安の幅であって統計的な信頼区間ではない**旨を、チャートの `figcaption` と
`/about` の2か所に置いている。**帯だけを見ると信頼区間に見える**ので、図の側の断りを
省けない。**表の `caption` には置かない**（Issue #95）——表・説明文・チャートは1つの
`section` に縦に続くので、表にも同じ文を置くと一度の視界に断りが2つ並ぶ。E2E が
「ページ内に1か所（`figcaption`）」「表に `caption` が無い」「`/about` にもある」ことを
固定している。
