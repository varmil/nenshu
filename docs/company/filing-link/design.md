...
**C16（#838）で4セルの表を「{社名}の年収に関するQ&A」の4問に作り替え、節を要約の直後へ移した。**
帯の中身は変えておらず、いまは4問の枠の下辺に付いている（`docs/company/actuals-qa/design.md`）。
決算期は見出しから Q&A の説明の1行へ移ったので、下の「決めたこと」の「真上の見出し」はいまは
Q&A の説明を指す。
