...
**`/company/[id]` の読み込みと計算は runtime 施策の R0（Issue #165）が減らした。** `logos.json`（202KB）を「ロゴがあるか」のために丸ごと読んでいたのを `logo-ids.json` に替え、`generateMetadata` と本体で2回走っていた `buildCompanyView` を React の `cache()` で1回にし、`findNeighbors` の全表走査（1リクエストで延べ33,606行）を isolate ごとの索引にした。**新しいデータファイルを足すときは ADR-0011 の問いを通ること**——C5〜C7（#159〜#161）の説明文がそれに当たる。
