...
**`/company/[id]` は runtime 施策の R1（Issue #180・親 #118）で全社を事前生成するようになった**（ADR-0012）。リクエスト時に描画しないので、**この画面に項目を足す Unit は `searchParams` を読めない**（W1・#150、C5〜C7・#159〜#161）。読みたくなったらそれは「このページを動的に戻す」という決定なので ADR-0012 を改訂すること。あわせて**表示基準（年齢そろえ）は URL に出さなくなった**——`?age=` は無く、クライアントの状態としてだけ持つ（spec 1.5）。
