...
| C9 | 要約と分析の生成 | C8 | AC-25〜AC-27 | オフラインのバッチ。web には出さない。Issue #241（親 #214） |
| C10 | 要約と分析の表示 | C9 | AC-28〜AC-29 | 2セクション。見分けられることが要件。Issue #242（親 #214） |
...
`［ADR-0015］` も Unit ではない。**分析セクションに限って評価を書いてよいと決める**ところで、`spec.md` 1.16 と 1.19・AC-24〜AC-29 の改訂を伴う（済み）。**C8 が C7 の後に来るのは順序の都合だけで、依存ではない**——原文の調達はパイプラインの中で閉じる（C5 と同じ型）。
...
**規格が2つある**のがこの Unit の要点になる（ADR-0015 決定1）。要約は C6 の機械ゲートをそのまま流用でき、分析は別に作る——**評価語は通し、代わりに算用数字の個数に上限を置く**。検証パスも「原文に書いてあるか」と「原文と数値から導けるか」で分かれる。

**1回の呼び出しで両方書かせる。** 原文が C6 の約20倍（1社あたり約14,400字）あり、2回読ませると量が倍になる。**量が制約になるのはここだけ**で、原文の切り方・使う節の数・数値の個数の上限は**60社のパイロットで実測して決める**（C6 が同じやり方で打ち切りを決めた）。
