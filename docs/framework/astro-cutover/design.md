...
2026-08-26 に main へマージし、本番（`https://openreport.net/`）で確認した。

**どのURLが Worker を起こすか**（`x-openreport-rendered` ヘッダ）:

| URL | 応答 | 印 |
| --- | --- | --- |
| `/`・`/?age=35`・`/?ind=銀行業` | 200 | **`worker`** |
| `/about`・`/company/6861`・`/company/E03362` | 200 | 無し（＝静的アセット） |
| `/sitemap.xml`・`/robots.txt` | 200 | 無し |
| `/company/does-not-exist`・存在しないパス | 404 | 無し |

**`x-nextjs-*` は1つも出なくなった**（切り替え前は `/about` と `/company/[id]` に `x-nextjs-cache: HIT` / `x-nextjs-prerender: 1,1` が付いていた）。

**中身**: 404 は `lang="ja"` の「ページが見つかりません」＋共通ヘッダ。`robots.txt` は `User-Agent: *` ／ `Allow: /` ／ sitemap 1行。`sitemap.xml` は 3,004 URL。タイトルは `/` が `OpenReport | 有価証券報告書ベースの平均年収ランキング 2,961社【2025年3月期〜2026年5月期】`、`/?age=35` が `35歳年収ランキング | OpenReport`、`/company/6861` が `株式会社キーエンスの平均年収 | 有価証券報告書は2,178万円`。

**HTML の大きさ**は `wrangler dev` で測った値と一致した（`/` raw 246,536 B・gzip 19,480 B、`/about` 48,221 B・12,950 B、`/company/6861` 144,084 B・19,546 B。差はヘッダ由来の 100〜130 B）。

**ブラウザで触る確認はできなかった**——このセッションのコンテナから本番へは Chromium が届かない（`ERR_CONNECTION_RESET`。`curl` は proxy を見るので通る）。同じビルド成果物に対する E2E（dev 381件・`wrangler dev` 31件）で代替した。

## デプロイ設定の切り替えで詰まった点

**ダッシュボードのビルドコマンドを変えても、ブランチのビルドには反映されない。** `npm run build` に変えて保存したあと、プッシュ3回と再試行1回のすべてが**旧コマンドで走って落ちた**（20〜37秒。`npm ci` の直後に `npm error could not determine executable to run`）。**効いたのは main にマージした後の本番ビルド。**

- **`wrangler.jsonc` と同じ振る舞い**（CLAUDE.md「Worker の設定は main に入れるまで効かない」・2026-08-21・#119）
- **本番が新しいコマンドで通った後も、ブランチのビルドは旧いまま落ちる。** この記録を足した PR（[#225](https://github.com/varmil/nenshu/pull/225)）自体がそうだった——**main との差は markdown 3ファイルだけ**で、その main は直前に本番ビルドを通っているのに、ブランチのビルドは14秒で落ちた（これまでの4回と同じ形）。**ブランチのビルドが赤いことをコードの問題と読まないこと**
- **「ビルドを再試行」はさらに効かない。** 元のビルド構成をそのまま再生する
- **どのコマンドで走ったかはビルド詳細の「ビルドの設定」欄で確かめられる。** ログの `Executing user build command:` の行と同じ
- **原因の切り分けに時間を使った。** ログが読めなかった（API トークンに Workers Builds のスコープが無く `12006 Invalid token`）ため、**Node のバージョンを疑って `.node-version` を入れ、外した**——ログを見たら環境の Node は最初から 24.18.0 だった。**設定変更が効いているかは、まずビルドの「ビルドの設定」欄を見ること**
